...
- Источник YouTube принимает конкретный `video_id`. Автоматическое обнаружение новых видео не реализовано: обновляйте `sources.json` либо добавляйте источник RSS канала для обнаружения и расширяйте коннектор.
